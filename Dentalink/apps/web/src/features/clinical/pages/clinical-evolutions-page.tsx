...
export function ClinicalEvolutionsPage() {
  ...
  const handleAnnul = () => {
    if (evolutionToAnnul && annulReason.trim()) {
      mutations.annulEvolution.mutate({ evolutionId: evolutionToAnnul, reason: annulReason.trim() }, {
        onSuccess: () => {
  ...
  return (
    <ClinicalShell patientId={id} title="Evoluciones clinicas" description="Registro clínico legal, firma electrónica y adendas.">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-200 pb-4 mb-6 gap-4">
        ...
        <div className="flex items-center gap-3">
          <Button variant="secondary" size="sm" className="h-9 text-slate-600 hover:text-slate-900 border-slate-300">
          ...
          <Button 
            ...
            onClick={() => { setSelectedEvolutionId(null); setIsModalOpen(true); }}
          >
      ...
      <div className="space-y-6">
        {evolutions.isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-slate-200 rounded-xl bg-slate-50">
            ...
            <p className="text-slate-500 mt-1 mb-4 max-w-sm">Este paciente no tiene evoluciones clínicas registradas. Crea la primera evolución para registrar procedimientos o atenciones.</p>
            <Button onClick={() => { setSelectedEvolutionId(null); setIsModalOpen(true); }}>Crear primera evolución</Button>
          </div>
        ) : null}
        
        {displayedEvolutions.map((evolution: any) => (
          <Card key={evolution.id} className={`overflow-hidden transition-all shadow-sm border ${evolution.annulledAt ? 'border-red-200 bg-red-50/30' : 'border-slate-200 hover:shadow-md'}`}>
            <div className={`px-6 py-4 border-b ${evolution.annulledAt ? 'bg-red-50/50 border-red-100' : 'bg-slate-50/80 border-slate-100'} flex flex-wrap items-center justify-between gap-4`}>
              <div className="flex items-center gap-4">
                <div className={`flex h-10 w-10 items-center justify-center rounded-full ${evolution.annulledAt ? 'bg-red-100 text-red-600' : evolution.signedAt ? 'bg-green-100 text-green-600' : 'bg-yellow-100 text-yellow-600'}`}>
                  {evolution.annulledAt ? <XCircle className="h-5 w-5" /> : evolution.signedAt ? <CheckCircle className="h-5 w-5" /> : <AlertTriangle className="h-5 w-5" />}
                </div>
                <div>
              ...
              <div className="flex items-center gap-2">
                {getStatusBadge(evolution)}
              </div>
            </div>
            
            <div className="p-6">
              {/* Contexto de Tratamiento */}
              {(evolution.actionNameSnapshot || (evolution.treatmentPlanItem && evolution.treatmentPlanItem.procedure)) && (
                <div className="mb-5 inline-block rounded-md bg-blue-50 px-3 py-1.5 text-sm font-medium text-blue-800 border border-blue-100">
                  ...
                  {evolution.treatmentPlanItem?.treatmentPlan && <span className="text-blue-600 ml-2 font-normal">(Plan #{evolution.treatmentPlanItem.treatmentPlan.displayId})</span>}
                </div>
              )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 mb-6 p-4 rounded-lg bg-slate-50 border border-slate-100">
                  {evolution.fields.map((field: any, idx: number) => (
                    <div key={idx} className="flex flex-col">
                      <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">{field.label}</span>
              ...
              {evolution.materials && evolution.materials.length > 0 && (
                <div className="mb-6">
                  <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Materiales utilizados</h4>
                  <ul className="space-y-1">
                    {evolution.materials.map((mat: any, idx: number) => (
                      <li key={idx} className="text-sm flex items-center gap-2 text-slate-700">
                        ...
                        <span className="font-medium">{mat.quantity} {mat.unitSnapshot || mat.inventoryItem?.unit || "unidades"}</span>
                        <span>de</span>
              ...
              {evolution.notes && (
                <div className="mb-2">
                  <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Evolución Clínica</h4>
                  <div 
              ...
              {evolution.addenda && evolution.addenda.length > 0 && (
                <div className="mt-6 space-y-3 border-t border-slate-100 pt-5">
                  ...
                  {evolution.addenda.map((addendum: any) => (
                    <div key={addendum.id} className="rounded-md bg-amber-50/50 p-3 border border-amber-100 text-sm">
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-medium text-amber-900">{addendum.professional?.firstName} {addendum.professional?.lastName}</span>
                        <span className="text-xs text-amber-700">{new Date(addendum.createdAt).toLocaleDateString()}</span>
                      </div>
              ...
              <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4">
                ...
                {!evolution.signedAt && (
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    <Button variant="secondary" size="sm" onClick={() => { setSelectedEvolutionId(evolution.id); setIsModalOpen(true); }}>
                      Editar Borrador
                    </Button>
                    <div className="flex items-center gap-1.5">
                      <Button className="bg-green-600 hover:bg-green-700 text-white" size="sm" onClick={() => void mutations.signEvolution.mutate(evolution.id)}>
                        Firmar Evolución
                      </Button>
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full justify-between">
                    <div className="flex w-full sm:w-2/3 items-start sm:items-center gap-2">
                      <Textarea 
                        ...
                        onChange={(event) => setAddendum((prev) => ({ ...prev, [evolution.id]: event.target.value }))} 
                      />
                      <Button
                        size="sm"
                        className="whitespace-nowrap bg-slate-800 text-white hover:bg-slate-700"
                        disabled={!addendum[evolution.id]?.trim()}
                        onClick={() => void mutations.createAddendum.mutate(
                          { evolutionId: evolution.id, professionalId: evolution.professionalId, notes: addendum[evolution.id] },
                    ...
                    <Button variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-50 text-sm font-medium" onClick={() => openAnnulModal(evolution.id)}>
                      Anular Evolución
                    </Button>
                  </div>
                )}
      ...
      <ClinicalEvolutionModal 
        open={isModalOpen} 
        onClose={() => { setIsModalOpen(false); setSelectedEvolutionId(null); }} 
        patientId={id}
        branchId={branchId}
      ...
      {/* Annul Modal */}
      <Modal open={isAnnulModalOpen} onClose={() => setIsAnnulModalOpen(false)} title="Anular Evolución" size="md">
        <div className="flex flex-col gap-4">
          <div className="rounded-md bg-red-50 p-4 border border-red-100 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-red-700 mt-1">La anulación revertirá los descuentos de inventario asociados y eliminará el estado "Completado" del plan de tratamiento (si aplica). Esta acción dejará un registro inmutable por auditoría médica.</p>
            </div>
          </div>
          
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700">Motivo de Anulación <span className="text-red-500">*</span></label>
            <Textarea 
              value={annulReason}
          ...
          <div className="flex items-center justify-end gap-3 mt-4">
            <Button variant="secondary" onClick={() => setIsAnnulModalOpen(false)}>Cancelar</Button>
            <Button variant="destructive" onClick={handleAnnul} disabled={!annulReason.trim() || mutations.annulEvolution.isPending}>
              Confirmar Anulación
            </Button>
          </div>
        </div>
