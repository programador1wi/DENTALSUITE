...
type InventoryView = "inventory" | "products" | "catalogs" | "movements" | "safety" | "warehouses";
type OperationType = "ENTRY" | "EXIT" | "TRANSFER" | "POSITIVE_ADJUSTMENT" | "NEGATIVE_ADJUSTMENT" | "WASTE" | "SALE";

const movementTypes: InventoryMovementType[] = [
  "ENTRY", "EXIT", "TRANSFER", "POSITIVE_ADJUSTMENT", "NEGATIVE_ADJUSTMENT", "WASTE",
  "SUPPLIER_RETURN", "RETURN_IN", "PHYSICAL_COUNT_ADJUSTMENT", "COMPENSATION", "IN", "OUT", "ADJUSTMENT"
];

function asInventoryView(value: string | null): InventoryView {
  if (value === "products" || value === "catalogs" || value === "movements" || value === "safety" || value === "warehouses") return value;
  return "inventory";
}
  ...
  return (
    <div className="flex flex-col gap-1 w-36 py-0.5">
      <div className="flex items-center justify-between gap-2 text-xs">
        <span className="font-semibold text-slate-900">{stockQty} <span className="text-[10px] text-slate-400 font-normal">/ mín {minQty}</span></span>
    ...
    <div ref={ref} className="relative inline-block text-left">
      ...
      {open && (
        ...
        >
          <button
            type="button"
            className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-slate-700 hover:bg-emerald-50 hover:text-emerald-700 transition-colors"
            onClick={() => {
              setOpen(false);
              onOpenOperation("ENTRY", row);
            }}
          >
          ...
          <button
            type="button"
            className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-slate-700 hover:bg-amber-50 hover:text-amber-700 transition-colors"
            onClick={() => {
              setOpen(false);
              onOpenOperation("EXIT", row);
            }}
          >
            <MinusCircle className="h-4 w-4 text-amber-600 shrink-0" />
            <span>Sacar Stock</span>
          </button>
          <button
            type="button"
            className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-slate-700 hover:bg-blue-50 hover:text-blue-700 transition-colors"
            onClick={() => {
              setOpen(false);
              onOpenOperation("TRANSFER", row);
            }}
          >
          ...
          <button
            type="button"
            className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-slate-700 hover:bg-rose-50 hover:text-rose-700 transition-colors"
            onClick={() => {
              setOpen(false);
              onOpenOperation("WASTE", row);
            }}
          >
          ...
          {row.isSellable ? (
            <button
              type="button"
              className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium text-slate-700 hover:bg-purple-50 hover:text-purple-700 transition-colors border-t border-slate-100 mt-1 pt-2"
              onClick={() => {
                setOpen(false);
                onOpenOperation("SALE", row);
              }}
            >
              <ShoppingCart className="h-4 w-4 text-purple-600 shrink-0" />
              <span>Vender en Caja</span>
            </button>
          ) : null}
        </div>
      )}
