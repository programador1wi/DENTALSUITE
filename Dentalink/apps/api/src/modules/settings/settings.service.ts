...
export class SettingsService {
  constructor(
    ...
    private readonly expensesService?: SettingsExpensesService
  ) {}

  private get expenses(): SettingsExpensesService {
    return this.expensesService ?? new SettingsExpensesService(this.prisma);
  }
  ...
  async getGeneral(actor: AuthUser) {
    ...
    return {
      ...
      logoUrl: safeLogoForPresentation(organization.logoUrl),
    ...
    const organization = await this.prisma.organization.update({
      ...
      data: {
        ...
        ...(dto.logoUrl !== undefined
          ? { logoUrl: normalizeLogoReference(dto.logoUrl, current.logoUrl) }
          : {})
      }
    });
  ...
  async upsertBrand(actor: AuthUser, id: string | undefined, dto: UpsertBrandDto) {
    ...
    const currentBrand = id
      ? await this.prisma.branchBrand.findFirst({ where: { id, organizationId: actor.organizationId } })
      : null;
    if (id && !currentBrand) throw new NotFoundException("Brand not found");
    const logoUrl = dto.logoUrl === undefined
      ? currentBrand?.logoUrl
      : normalizeLogoReference(dto.logoUrl, currentBrand?.logoUrl);
    if (!id) {
  ...
  async getExpenseSummary(actor: AuthUser, branchId?: string, month?: number, year?: number) {
    return this.expenses.getExpenseSummary(actor, branchId, month, year);
  }
  ...
  async voidExpense(actor: AuthUser, expenseId: string, dto: VoidExpenseDto) {
    return this.expenses.voidExpense(actor, expenseId, dto);
  }

  async getExpense(actor: AuthUser, expenseId: string) {
    return this.expenses.getExpense(actor, expenseId);
  }
